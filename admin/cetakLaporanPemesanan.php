...
$statusPembayaran  = $_GET['status_pembayaran'];
$tglSaatIni = "";
$totalTransaksi = 0;
if ($tglDari!= null && $tglHingga != null) {
    ...
    $getData = mysqli_query($conn, "SELECT * FROM tb_pemesanan WHERE created_at BETWEEN '$tglDari' AND DATE_ADD('$tglHingga', INTERVAL 1 DAY) AND status_pemesanan !='P' AND status_pembayaran='$statusPembayaran'  ORDER BY tb_pemesanan.id ASC");
} else {
    $tglSaatIni = date('F Y');
...
$html = '
<h1 style="text-align:center; margin-bottom:10px;">Rekap Data </h1>';
...
<tr>
<th class="no">No</th>
<th class="nama_pemesan">Nama Pemesan</th>
<th class="nama_pemesan">Nama Kamar</th>
<th class="nama_pemesan">Harga Kamar</th>
<th class="nama_pemesan">Status Pemesanan</th>
<th class="nama_pemesan">Status Pembayaran</th>
<th class="nama_pemesan">Tanggal Pemesanan</th>
</tr>';
$no = 1;
while ($dataPesanan = mysqli_fetch_array($getData)) {
    ...
    $statusPemesanan ='';
    if ($dataPesanan['status_pemesanan']=='A') {
        $statusPemesanan = 'Terkonfirmasi';
    }elseif($dataPesanan['status_pemesanan']=='B'){
        $statusPemesanan = 'Dibatalkan';
    }elseif($dataPesanan['status_pemesanan']=='R'){
    ...
    <td class="border">Rp.'.number_format($dataPesanan['jumlah'], 0, ',', '.').'</td>
    <td class="border">'.$statusPemesanan.'</td>
    <td class="border">'.($dataPesanan['status_pembayaran'] == 'L' ? 'Lunas' : 'Belum Lunas').'</td>
